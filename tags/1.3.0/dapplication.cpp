...
/*
void DApplication :: OpenDir(){

    dbg_printf("open dir \n");
    system("/usr/SYSqtapp/mystuff/mystuff");
}
*/
void DApplication :: OpenFile(QListBoxItem *item,const QPoint &){

    if(item!=NULL)
    {
        dbg_printf("open file is name %s\n",(DirPath+"/"+item->text()).latin1());
        QByteArray ba1 = (DirPath+"/"+item->text()).local8Bit();
        system("/usr/SYSqtapp/mediaeditor/mediaeditor -d " + ba1);
    }
}

void DApplication :: setDirPath(const QString &str){

    DirPath=str;
}

void  DApplication :: timerEvent(QTimerEvent *te)
{
    if (te->timerId() == startTimer(1000)) //更新时钟，每秒一次
    {
        printf("timer event\n");
    }
    printf("timer event is %d\n",te->timerId());
}
...
bool DApplication :: qwsEventFilter(QWSEvent *event)
 {
     ...
     switch(event->type) {
         ...
         case QWSEvent::Key:
             ...
             if (KeyEvent->simpleData.is_press && KeyEvent->simpleData.is_auto_repeat) {
                 ...
                 if(keypress)
                 {
                     ...
                     if(pressedtime == 3)
                     {
                         pressedtime = 0;
                         keypress = false;
                         pointerListener->keyPresslong(kcode);
                     }
                     pressedtime++;
                 }
             }
             ...
             if (!KeyEvent->simpleData.is_press && !KeyEvent->simpleData.is_auto_repeat) {
                 if(!keypress && pressedtime != 3)
                 {
                     //pointerListener->keyPresslong(KeyEvent->simpleData.keycode);
                 }else{
                     pointerListener->keyPressed(kcode);
                 }
                 keypress = true;
             }
         break;
...
/*
         case QWSEvent::PropertyNotify:
             //PropertyNotify = (QWSPropertyNotifyEvent*) event;
             if (propertyNotify==0)
                 return 0;
